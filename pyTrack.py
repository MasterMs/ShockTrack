...
async def index():
    ...
    <html>
    <body>
        <div id="v" style="font-size:40px; white-space:pre-wrap; font-family: monospace;">(waiting for data)</div>
        <div style="margin-top:20px;">
            <button id="reconnect">Check / Reconnect</button>
            <span id="status" style="margin-left:10px;">status: unknown</span>
        </div>
        ...
        <script>
            ...
            function connectWS(){
                ...
                ws.onmessage = (msg) => {
                    // display the raw comma-separated line
                    valueEl.textContent = msg.data;
                };

                ws.onclose = () => {
                    console.log('ws closed');
                    statusEl.textContent = 'status: disconnected (websocket)';
                    // keep the UI responsive; allow manual reconnect
                };
            ...
            async function checkStatus(){
                try{
                    ...
                    statusEl.textContent = 'status: ' + (j.connected ? 'device connected' : 'device disconnected');
                    if (j.connected && (!ws || ws.readyState !== WebSocket.OPEN)){
                        connectWS();
                    }
                }catch(e){
            ...
            // regularly poll status (every 5s)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("pyTrack_with_logging:app",
